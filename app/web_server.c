...
int main(int argc, char const *argv[]) {
    ...
    printf("Opening TCP port %d...\n", HTTP_LISTEN_PORT);
    if (tcp_open(HTTP_LISTEN_PORT, http_request_handler) == -1) {
        printf("Failed to open TCP port %d.\n", HTTP_LISTEN_PORT);
        return -1;
    }
    printf("TCP port %d opened successfully.\n", HTTP_LISTEN_PORT);

    printf("Web server is now running...\n");
    while (1) {
        net_poll();  // 一次主循环
    }
